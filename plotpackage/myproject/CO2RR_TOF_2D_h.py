...
if __name__ == '__main__':
    ...
    for i,metal in enumerate(metals):
        pl.plot(ECO_d[metal], EHOCO_d[metal], 'o', color='black') 
        pl.text(ECO_d[metal], EHOCO_d[metal]+0.05, metal, fontsize=12, horizontalalignment='center', verticalalignment='bottom')
    ...
    pl.axline((list(ECO_d.values())[0], list(ECO_d.values())[0]*m +b), slope=m, color='black')
    pl.xlim([E_CO_e[0]+0.1, E_CO_e[-1]-0.1])
    pl.ylim([E_HOCO_e[0]+0.1, E_HOCO_e[-1]-0.1])
    pl.colorbar(ticks=np.arange(min(contours), max(contours), 0.5))
    pl.title(r'log$_{10}$(j/$\mu$Acm$^{-2}$)')
    pl.xlabel(r'$E_{\mathrm{CO}}$ (eV)')
    pl.ylabel(r'$E_{\mathrm{HOCO}}$ (eV)')
    pl.savefig('../data/CO2toCO_rate_vs_HOCO_CO.png', dpi=300)
    pl.show()
